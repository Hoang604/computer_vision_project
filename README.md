...
│   └── hr_images/                 # (User-created) Example subdirectory with HR images (e.g., for train, validation)
│
...
2.  **Create a virtual environment (recommended):**
    ...
    **Alternatively, using Conda:**
    ```bash
    conda create -n myenv python=3.12 # Or your preferred Python version
...
*Note*: This script do not split data into train, test, and validation set. You need to process train, test, and validation set independently
...
#### Pipeline A: Refining Bicubic Upscaling (using `scripts/train_diffusion_bicubic_refine.py`)
...
```
**Key Arguments for `scripts/train_diffusion_bicubic_refine.py`:** (Refer to the script's `argparse` for full details)
...
#### Pipeline B: Refining RRDBNet Upscaling (using `scripts/train_diffusion_rrdb_refine.py`)
...
```
**Key Arguments for `scripts/train_diffusion_rrdb_refine.py`:** (Refer to script's `argparse`)
