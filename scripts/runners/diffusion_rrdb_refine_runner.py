...
def run_diffusion_rrdb_refine_training(config):
    """
    ...
    assert context_mode_for_trainer in [
        'LR', 'HR'], "Context mode for trainer must be either 'LR' or 'HR'."

    # --- Setup Device ---
    if config.device.startswith("cuda") and not torch.cuda.is_available():
        print(
            f"CUDA device {config.device} requested but CUDA not available. Using CPU.")
        device = torch.device("cpu")
    elif not config.device.startswith("cuda") and config.device != "cpu":
        print(
            f"Invalid device specified: {config.device}. Using CPU if CUDA not available, else cuda:0.")
    ...
    print(
        f"Loading preprocessed data (LR, HR_RRDB, HR_Orig) from: {config.data_folder}")
    ...
    val_loader = None
    if config.val_data_folder:
        print(
            f"Loading validation data from: {config.val_data_folder}")
        val_dataset = ImageDataset(
            folder_path=config.val_data_folder,
            img_size=config.img_size,
            downscale_factor=config.downscale_factor
        )
    ...
    if not config.rrdb_weights_path_context_extractor or not os.path.exists(config.rrdb_weights_path_context_extractor):
        raise FileNotFoundError(
            f"RRDBNet weights for context extractor not found at: {config.rrdb_weights_path_context_extractor}")
    ...
    print(
        f"UNet model initialized with base_dim={config.unet_base_dim}, dim_mults={tuple(config.unet_dim_mults)}, use_attention={config.use_attention}")
    print(
        f"UNet cond_dim={config.rrdb_num_feat_context}, cond_rrdb_num_blocks={config.rrdb_num_block_context}")
    ...
    print(
        f"Optimizer: AdamW, LR: {config.learning_rate}, Weight Decay: {config.weight_decay}")
    ...
    if config.scheduler_type.lower() != "none":
        if config.scheduler_type.lower() == "steplr":
            ...
            print(
                f"Using StepLR scheduler with step_size_epochs={config.lr_decay_epochs}, gamma={config.lr_gamma}")
            ...
            print(
                f"Using CosineAnnealingLR scheduler with T_max_epochs={t_max_epochs_for_scheduler}, eta_min={config.eta_min_lr}")
            ...
            print(
                f"Using ExponentialLR scheduler with gamma={config.lr_gamma}")
        else:
            print(
                f"Warning: Unknown scheduler type '{config.scheduler_type}'. No scheduler will be used.")
            config.scheduler_type = "none"

    # --- Load Checkpoint if available ---
    start_epoch = 0
    best_loss = float('inf')
    weights_path_unet = config.weights_path_unet if config.weights_path_unet and os.path.exists(
        config.weights_path_unet) else None
    if weights_path_unet:
        ...
        print(
            f"Loaded UNet checkpoint. Resuming from epoch {start_epoch}, best loss: {best_loss:.6f}")
    else:
    ...
    try:
        diffusion_trainer.train(
            ...
            is_learning_residual=config.is_learning_residual,
        )
