...
import os
import argparse
from diffusion_modules import Unet, RRDBNet
...
def train_diffusion(args):
    ...
    device = args.device
    # --- Setup ---
    device = torch.device(device if torch.cuda.is_available() and torch.cuda.device_count() > 1 else "cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    if args.weights_path is not None and os.path.exists(args.weights_path):
        start_epoch, best_loss = diffusion_trainer.load_checkpoint_for_resume(device='cuda', model=unet_model, optimizer=optimizer, checkpoint_path=args.weights_path)
    ...
    # --- Initialize the context_extractor RRBDNet ---
    context_extractor = RRDBNet(
        in_channels=args.img_channels,
        out_channels=args.img_channels,
        rrdb_in_channels=args.rrdb_in_channels,
        number_of_rrdb_blocks=args.number_of_rrdb_blocks,
    )
...
if __name__ == "__main__": 
    ...
    # Dataset args
    parser.add_argument('--img_size', type=int, default=160, help='Target image size')
    parser.add_argument('--img_channels', type=int, default=3, help='Number of image channels')
    parser.add_argument('--image_folder', type=str, default="/media/tuannl1/heavy_weight/data/cv_data/images160x160", help='Path to the image folder (hardcoded in the script)')
    parser.add_argument('--downscale_factor', type=int, default=4, help='Downscale factor for original image size')
    # Training args
    parser.add_argument('--rrdb_in_channels', type=int, default=64, help='Input channels for RRDBNet')
    parser.add_argument('--number_of_rrdb_blocks', type=int, default=8, help='Number of RRDB blocks in RRDBNet')
    parser.add_argument('--eightbit', type=bool, default=False, help='Use 8-bit AdamW optimizer')
    ...
    parser.add_argument('--weights_path', type=str, default="/home/hoangdv/cv/cv_checkpoints/v_prediction_20250513-172729/diffusion_model_v_prediction_best.pth", help='Path to pre-trained model weights')
    parser.add_argument('--diffusion_mode', type=str, default='v_prediction', help='Diffusion mode (v_prediction or noise)')
    # UNet args
    parser.add_argument('--num_resnet_block', type=int, default=2, help="Number of resnet block in each U-net layer")
    ...
    # Logging/Saving args
    parser.add_argument('--base_log_dir', type=str, default='/home/hoangdv/cv/cv_logs', help='Base directory for logging')
    parser.add_argument('--base_checkpoint_dir', type=str, default='/home/hoangdv/cv/cv_checkpoints', help='Base directory for saving checkpoints')
    parser.add_argument('--continue_log_dir', type=str, default="/home/hoangdv/cv/cv_logs/v_prediction_20250513-172729", help='Directory for continue logging on old TensorBoard logs')
    parser.add_argument('--continue_checkpoint_dir', type=str, default="/home/hoangdv/cv/cv_checkpoints/v_prediction_20250513-172729", help='Directory for continue training on old checkpoints')
    # Loading model args
    ...
    print(f"Log Directory Base: {args.base_log_dir}") 
    print(f"Checkpoint Directory Base: {args.base_checkpoint_dir}") 
    print(f"--------------------") 
