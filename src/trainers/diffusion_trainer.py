...
class DiffusionTrainer:
    ...
    def __init__(
        ...
        self.posterior_variance = torch.nan_to_num(
    ...
    def _setup_training_directories_and_writer(self, log_dir_base, checkpoint_dir_base,
        ...
        if checkpoint_dir_param:
            # Use the provided checkpoint_dir to resume
        ...
        best_checkpoint_path = os.path.join(
            checkpoint_dir, f'diffusion_model_{os.path.basename(checkpoint_dir)}_best.pth')
    ...
    def _initialize_training_steps(self, start_epoch, dataset_len, accumulation_steps):
        ...
        global_step_optimizer = start_epoch * \
            effective_batches_per_epoch  # Global optimizer steps
    ...
    def _get_training_target(self, noise_added, original_residual_batch, t):
        ...
        if self.mode == "v_prediction":
            ...
            sqrt_alphas_cumprod_t = self._extract(
                self.sqrt_alphas_cumprod, t, original_residual_batch.shape)
            sqrt_one_minus_alphas_cumprod_t = self._extract(
                self.sqrt_one_minus_alphas_cumprod, t, original_residual_batch.shape)
            target = sqrt_alphas_cumprod_t * noise_added - \
                sqrt_one_minus_alphas_cumprod_t * original_residual_batch
        elif self.mode == "noise":
            # For noise prediction (epsilon-prediction), target is the noise itself
            target = noise_added
        else:
            # Should not happen if validated in __init__
    ...
    def _perform_batch_step(self, model, context_extractor, batch_data, optimizer,
        ...
        low_res_image_batch, _, original_hr_batch, _ = batch_data
        low_res_image_batch = low_res_image_batch.to(self.device)
        original_hr_batch = original_hr_batch.to(self.device)
        ...
        if context_extractor is not None:
            ...
            with torch.no_grad():  # Feature extraction should not require gradients
                rrdb_upscaled_batch, raw_features_list_gpu = context_extractor(
                    low_res_image_batch, get_fea=True)
                condition_features_list = [
                    feat.to(self.device) for feat in raw_features_list_gpu]
        else:
            print(
        ...
        with torch.set_grad_enabled(is_training):
            t = torch.randint(0, self.timesteps, (actual_batch_size,),
                              device=self.device, dtype=torch.long)
            target_batch_t, noise_added = self.q_sample(
                target_batch, t)  # Noise the target residual
            target_for_unet = self._get_training_target(
                noise_added, target_batch, t)  # Get U-Net's target

            predicted_output = model(
                target_batch_t, t, condition=condition_features_list)
    ...
    def _run_validation_epoch(self, model, context_extractor, val_loader, writer, epoch):
        ...
        progress_bar_val = tqdm(total=len(val_loader),
        ...
        with torch.no_grad():  # No gradients needed for validation
            for batch_idx, batch_data in enumerate(val_loader):
                ...
                progress_bar_val.set_postfix(
                    val_loss_batch=f"{loss_value:.4f}")

        progress_bar_val.close()
        avg_val_loss = total_val_loss / \
            num_val_batches if num_val_batches > 0 else float('nan')
        print(
            f"Epoch {epoch+1} Average Validation Loss ({self.mode}): {avg_val_loss:.4f}")

        if writer:
            writer.add_scalar(
                f'Loss_{self.mode}/validation_epoch_avg', avg_val_loss, epoch + 1)
        return avg_val_loss
    ...
    def _log_and_checkpoint_epoch_end(self, epoch, model, optimizer, scheduler, train_epoch_losses,
        ...
        mean_train_loss = np.mean(
            train_epoch_losses) if train_epoch_losses else float('nan')
        print(
            f"Epoch {epoch+1} Average Training Loss ({self.mode}): {mean_train_loss:.4f}")
        writer.add_scalar(
            f'Loss_{self.mode}/train_epoch_avg', mean_train_loss, epoch + 1)

        if optimizer.param_groups:
            current_lr = optimizer.param_groups[0]['lr']
            writer.add_scalar(f'LearningRate/epoch', current_lr, epoch + 1)
            print(
                f"Current Learning Rate at end of epoch {epoch+1}: {current_lr:.2e}")
        ...
        if loss_for_comparison is not None and not np.isnan(loss_for_comparison) and loss_for_comparison < current_best_val_loss:
            ...
            print(
                f"Saved new best model checkpoint to {best_checkpoint_path} (Epoch {epoch+1}, Best Loss: {new_best_val_loss:.4f}, Train Loss: {mean_train_loss:.4f})")

        # Save a checkpoint for the current epoch (non-best)
        # This is useful for resuming if training is interrupted.
        current_epoch_checkpoint_path = os.path.join(os.path.dirname(
            best_checkpoint_path), f'diffusion_model_epoch_{epoch + 1}.pth')
        ...
        torch.save(current_epoch_checkpoint_data,
                   current_epoch_checkpoint_path)
        print(
            f"Saved checkpoint for epoch {epoch + 1} to {current_epoch_checkpoint_path}")
        ...
        if scheduler:
            if isinstance(scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                ...
                if metric_for_plateau is not None and not np.isnan(metric_for_plateau):
                    ...
                    print(
                        f"ReduceLROnPlateau scheduler stepped with metric {metric_for_plateau:.4f}.")
                else:
                    print(
                        "ReduceLROnPlateau scheduler not stepped as metric is None or NaN.")
            # For other schedulers like CosineAnnealingLR, StepLR (if epoch-based)
            else:
                scheduler.step()
                print(
                    f"Epoch-based scheduler stepped. New LR (from optimizer): {optimizer.param_groups[0]['lr']:.2e}")

        if dataset_for_samples is not None:  # Generate samples
            try:
                self._generate_and_log_samples(
                    model, dataset_for_samples, epoch, writer, context_extractor, is_learning_residual)  # Pass context_extractor
    ...
    # Added context_extractor
    def _generate_and_log_samples(self, model, dataset_loader, epoch, writer, context_extractor, is_learning_residual):
        """
        ...
        """
        print(
            f"Generating sample images for TensorBoard at epoch {epoch+1}...")
        ...
        try:
            ...
            print(
                "Warning: ResidualGenerator class not found. Skipping sample generation.")
            model.train()  # Revert model to train mode
            return
        except Exception as e_gen_init:
            print(
                f"Error initializing ResidualGenerator: {e_gen_init}. Skipping sample generation.")
            model.train()
            return
        ...
        num_samples_to_generate = min(
            3, dataset_loader.batch_size if dataset_loader.batch_size else 3)
        if num_samples_to_generate == 0:
            print(
                "Warning: Cannot generate samples, effective num_samples_to_generate is 0.")
            model.train()
            return
        ...
        original_b = original_b[:num_samples_to_generate].to(
            self.device)  # For display
        residual_b_gt = residual_b_cpu[:num_samples_to_generate].to(
            self.device)  # Ground truth residual

        # --- On-the-fly Feature Extraction for samples ---
        sample_condition_features = None
        if context_extractor is not None:
            with torch.no_grad():
                _, raw_features_list_gpu_sample = context_extractor(
                    low_res_b, get_fea=True)
                sample_condition_features = [
                    feat.to(self.device) for feat in raw_features_list_gpu_sample]
        else:
            # This should ideally not be None if U-Net is conditional
            print(
                "Warning: context_extractor is None during sample generation. Passing None condition to U-Net.")
        ...
        if is_learning_residual:
            for i in range(generated_residuals_batch.shape[0]):
                ...
                predicted_residual_display = (
                    generated_residuals_batch[i].cpu().numpy() + 1.0) / 2.0
                final_hr_constructed = torch.clamp(
                    up_scale_b[i] + generated_residuals_batch[i], -1.0, 1.0)
                final_hr_constructed_display = (
                    final_hr_constructed.cpu().numpy() + 1.0) / 2.0

                # Residual_b_gt is [-2,2]
                true_residual_display = (
                    residual_b_gt[i].cpu().numpy() + 1.0) / 2.0
            ...
            for i, imgs_dict in enumerate(sample_images_data):
                writer.add_image(
                    f'Sample_{i}/01_LowRes_Input', imgs_dict["low_res"], epoch + 1, dataformats='CHW')
                writer.add_image(
                    f'Sample_{i}/02_HR_RRDB_Base', imgs_dict["hr_rrdb"], epoch + 1, dataformats='CHW')
                writer.add_image(f'Sample_{i}/03_Predicted_Residual_Diffusion',
                                 imgs_dict["generated_residual"], epoch + 1, dataformats='CHW')
                writer.add_image(f'Sample_{i}/04_Final_HR_Refined',
                                 imgs_dict["final_hr_refined"], epoch + 1, dataformats='CHW')
                writer.add_image(f'Sample_{i}/05_Original_HR_GroundTruth',
                                 imgs_dict["original_hr_gt"], epoch + 1, dataformats='CHW')
                writer.add_image(f'Sample_{i}/06_True_Residual_GT_(HR_orig-HR_rrdb)',
                                 imgs_dict["true_residual_gt"], epoch + 1, dataformats='CHW')
        else:
            for i in range(generated_residuals_batch.shape[0]):
                lr_img_display = (low_res_b[i].cpu().numpy() + 1.0) / 2.0
                # This is HR_RRDB
            ...
            for i, imgs_dict in enumerate(sample_images_data):
                ...
                writer.add_image(
                    f'Sample_{i}/02_HR_RRDB_Base', imgs_dict["hr_rrdb"], epoch + 1, dataformats='CHW')
                writer.add_image(f'Sample_{i}/04_Final_HR_Generated',
                                 imgs_dict["final_hr_generated"], epoch + 1, dataformats='CHW')
                writer.add_image(f'Sample_{i}/05_Original_HR_GroundTruth',
                                 imgs_dict["original_hr_gt"], epoch + 1, dataformats='CHW')

        print(
            f"Logged {len(sample_images_data)} sample images to TensorBoard.")
        model.train()  # Revert model to train mode
    ...
    def train(self,
        ...
        print(
            f"Starting training in '{self.mode}' mode on device: {self.device} with {accumulation_steps} accumulation steps.")
        if context_extractor:
            print(
                f"Using on-the-fly context extraction with: {type(context_extractor).__name__}")
        print(f"Logging to: {log_dir}")
        print(f"Saving best checkpoints to: {best_checkpoint_path}")
        if val_dataset:
            print(
                f"Validation will be performed every {val_every_n_epochs} epoch(s).")
        print(
            f"Initial global optimizer steps: {global_step_optimizer}, initial batch steps: {batch_step_counter}")
        print(
            f"Initial best tracked loss (from resume or inf): {current_best_val_loss_tracker:.6f}")
        ...
        for epoch in range(start_epoch, epochs):
            ...
            progress_bar_train = tqdm(
                total=len(train_dataset), desc=f"Training ({self.mode}, Epoch {epoch+1})")
            train_epoch_losses = []
            ...
            for batch_idx, batch_data in enumerate(train_dataset):
                ...
                writer.add_scalar(
                    f'Loss_{self.mode}/train_batch_step_raw', loss_value, batch_step_counter)
                if updated_optimizer and optimizer.param_groups:
                    writer.add_scalar(f'LearningRate/optimizer_step',
                                      optimizer.param_groups[0]['lr'], global_step_optimizer)

                progress_bar_train.update(1)
                progress_bar_train.set_postfix(
                    loss=f"{loss_value:.4f}", opt_steps=global_step_optimizer, lr=f"{optimizer.param_groups[0]['lr']:.2e}")
            ...
            if val_dataset and (epoch + 1) % val_every_n_epochs == 0:
                current_val_loss_for_this_epoch = self._run_validation_epoch(
                    model, context_extractor, val_dataset, writer, epoch)
            ...
            current_best_val_loss_tracker = self._log_and_checkpoint_epoch_end(
                ...
                # Pass this epoch's validation loss (can be None)
        ...
        if current_accumulation_idx > 0:  # If training ended mid-accumulation cycle
            print(
                f"Performing final optimizer step for {current_accumulation_idx} remaining accumulated gradients...")
            optimizer.step()
            optimizer.zero_grad()
            global_step_optimizer += 1
            print(
                f"Final gradients applied. Total optimizer steps: {global_step_optimizer}")

        writer.close()
        print(
            f"Training finished for mode '{self.mode}'. Final best tracked loss: {current_best_val_loss_tracker:.4f}")
    ...
    def load_diffusion_unet(model_path: str,
        ...
        if 'model_config' not in loaded_checkpoint:
            raise KeyError(
                "Checkpoint missing 'model_config'. Cannot initialize UNet.")
        if 'model_state_dict' not in loaded_checkpoint:
            raise KeyError(
                "Checkpoint missing 'model_state_dict'. Cannot load UNet weights.")

        model_config = loaded_checkpoint['model_config']
        model_state_dict = loaded_checkpoint['model_state_dict']

        # Assuming this import path is correct
    ...
    def load_model_weights(model, model_path, verbose=False, device='cuda'):
        ...
        if not os.path.exists(model_path):
            # Log warning
            print(
                f"Warning: Model weights path not found: {model_path}. Model weights not loaded.")
            return

        print(
            f"Loading model weights from: {model_path} onto device: {device}")
        ...
        if isinstance(checkpoint, dict):
            if "model_state_dict" in checkpoint:
                ...
                is_likely_model_state_dict = all(not k.startswith(
                    ('optimizer', 'scheduler', 'epoch', 'loss')) for k in checkpoint.keys())
                if is_likely_model_state_dict:
                    ...
                    print(
                        f"Warning: Checkpoint dictionary does not contain a clear model state_dict. Keys: {list(checkpoint.keys())}")

        elif isinstance(checkpoint, torch.nn.Module):  # If the entire model was saved
            state_dict_to_load = checkpoint.state_dict()
            # Log full model saved
            print("Checkpoint was a full model object; extracting state_dict.")
        else:
            # Log warning
            print(
                f"Warning: Checkpoint format not recognized or is not a dictionary/model. Type: {type(checkpoint)}")
            return

        if state_dict_to_load:
            model.to(device)
            incompatible_keys = model.load_state_dict(
                state_dict_to_load, strict=False)  # Load with strict=False
            if incompatible_keys.missing_keys:
                # Log missing keys
                print(
                    f"Warning: {len(incompatible_keys.missing_keys)} keys in the current model were not found in the checkpoint.")
                if verbose:
                    # Verbose log
            ...
            if incompatible_keys.unexpected_keys:
                # Log unexpected keys
                print(
                    f"Info: {len(incompatible_keys.unexpected_keys)} keys in the checkpoint were not used by the current model.")
                if verbose:
                    # Verbose log
                    print(
                        f"Unused (unexpected) keys: {incompatible_keys.unexpected_keys}")

            if not incompatible_keys.missing_keys and not incompatible_keys.unexpected_keys:
                print(
                    f"Successfully loaded all parameters into the model on device {device}.")
            else:
                num_loaded_params = sum(1 for k_model in model.state_dict(
                ) if k_model in state_dict_to_load and k_model not in incompatible_keys.unexpected_keys)
                # Log success
                print(
                    f"Successfully loaded {num_loaded_params} compatible parameters into the model on device {device}.")
        else:
            # Log failure
            print(
                f"Warning: Could not extract a loadable state_dict from {model_path}.")

    @staticmethod
    # Added scheduler and verbose_load
    def load_checkpoint_for_resume(device, model, optimizer, scheduler, checkpoint_path, verbose_load=False):
        ...
        """
        start_epoch = 0
        # This should be the best loss tracked so far
        loaded_best_loss = float('inf')

        model.to(device)
        print(
            f"Attempting to load checkpoint for resume from: {checkpoint_path} onto device: {device}")

        if not os.path.isfile(checkpoint_path):
            print(
                f"Checkpoint file not found at {checkpoint_path}. Training will start from scratch.")
        ...
        try:
            ...
            print(
                f"Checkpoint dictionary loaded successfully from {checkpoint_path}.")

            # Load model state
            if 'model_state_dict' in checkpoint:
                DiffusionTrainer.load_model_weights(
                    model, checkpoint_path, verbose=verbose_load, device=device)
                print(f"Model state loaded for resume via load_model_weights.")
            else:
                print(
                    "Warning: 'model_state_dict' not found in checkpoint. Model weights not loaded for resume.")
                    ...
                    for state in optimizer.state.values():
                        for k, v in state.items():
                            if isinstance(v, torch.Tensor):
                                state[k] = v.to(device)
                    print(
                        f"Optimizer state loaded successfully for resume and moved to {device}.")
                except Exception as optim_load_err:
                    print(
                        f"Error loading optimizer state for resume: {optim_load_err}. Optimizer will start from scratch.")
            elif optimizer is None:
                print(
                    "Warning: Optimizer not provided, skipping optimizer state loading for resume.")
            else:
                print(
                    "Warning: 'optimizer_state_dict' not found in checkpoint. Optimizer starts from scratch for resume.")
            ...
            if 'scheduler_state_dict' in checkpoint and scheduler is not None:
                try:
                    ...
                    print(
                        f"Error loading scheduler state for resume: {sched_load_err}. Scheduler may start from scratch or use loaded optimizer LR.")
            elif scheduler is None:
                print(
                    "Info: Scheduler not provided, skipping scheduler state loading for resume.")
            else:
                print(
                    "Info: 'scheduler_state_dict' not found in checkpoint. Scheduler may start from scratch.")
            ...
            if 'epoch' in checkpoint:
                ...
                print(
                    "Warning: Epoch number not found in checkpoint. Starting from epoch 0 for resume.")

            if 'loss' in checkpoint:  # This should be the 'best_loss' tracked so far
                loaded_best_loss = checkpoint['loss']
                print(
                    f"Loaded 'best_loss' from checkpoint for resume: {loaded_best_loss:.6f}")
            elif 'best_loss' in checkpoint:  # If a more specific key exists
                loaded_best_loss = checkpoint['best_loss']
                print(
                    f"Loaded 'best_loss' (specific key) from checkpoint for resume: {loaded_best_loss:.6f}")
            else:
                print(
                    "Info: Best loss value not found in checkpoint. Using default best_loss (inf) for resume.")

            if 'global_optimizer_steps' in checkpoint:
                print(
                    f"Checkpoint saved with global_optimizer_steps: {checkpoint['global_optimizer_steps']}")

        except Exception as e:
            print(
                f"Error loading checkpoint for resume: {e}. Training will start from scratch.")
