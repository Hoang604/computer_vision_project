...
class BasicRRDBNetTrainer:
    def __init__(self,
                 # {'in_nc':3, 'out_nc':3, 'num_feat': 64, 'num_block': 16, 'gc': 32, 'sr_scale': 4}
                 model_config: dict,
                 # {'lr': 0.0002, 'beta1': 0.9, 'beta2': 0.999}
                 optimizer_config: dict,
                 # Example: {'type': 'CosineAnnealingLR', 't_max': 100, 'eta_min': 0}
                 scheduler_config: dict = None,
                 # {'exp_name': 'my_exp', 'log_dir_base': 'logs', 'checkpoint_dir_base': 'ckpts'}
        ...
        self.scheduler_config = scheduler_config if scheduler_config else {
            'type': 'none'}  # Default to no scheduler
    ...
    def _build_scheduler(self, optimizer_to_schedule):
        ...
        if scheduler_type == 'steplr':
            ...
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer_to_schedule,
                                                        ...
                                                        gamma=gamma)
            print(
                f"  StepLR configured with step_size={step_size}, gamma={gamma}")
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_to_schedule,
                                                                   ...
                                                                   eta_min=eta_min)
            print(
                f"  CosineAnnealingLR configured with T_max={t_max}, eta_min={eta_min}")
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer_to_schedule,
                                                                             ...
                                                                             eta_min=eta_min)
            print(
                f"  CosineAnnealingWarmRestarts configured with T_0={t_0}, T_mult={t_mult}, eta_min={eta_min}")
            ...
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer_to_schedule,
                                                                   ...
                                                                   verbose=verbose)
            print(
                f"  ReduceLROnPlateau configured with mode={mode}, factor={factor}, patience={patience}")
        else:
            print(
                f"Warning: Scheduler type '{scheduler_type}' not recognized. No scheduler will be used.")
            return None
        return scheduler
    ...
    def _setup_logging_and_checkpointing(self, log_dir_param, checkpoint_dir_param):
        ...
        log_dir_base = self.logging_config.get(
            'log_dir_base', 'logs_rrdb_basic_standalone')
        checkpoint_dir_base = self.logging_config.get(
            'checkpoint_dir_base', 'checkpoints_rrdb_basic_standalone')

        self.log_dir = log_dir_param if log_dir_param else os.path.join(
            log_dir_base, exp_name)
        self.checkpoint_dir = checkpoint_dir_param if checkpoint_dir_param else os.path.join(
            checkpoint_dir_base, exp_name)
        ...
        self.best_checkpoint_path = os.path.join(
            self.checkpoint_dir, 'rrdb_model_best.pth')

        print(f"Basic RRDBNet (Standalone) - Logging to: {self.log_dir}")
        print(
            f"Basic RRDBNet (Standalone) - Saving checkpoints to: {self.checkpoint_dir}")
        print(
            f"Basic RRDBNet (Standalone) - Best model will be saved to: {self.best_checkpoint_path}")
    ...
    def _run_validation_epoch(self, val_loader: DataLoader, epoch: int, predict_residual: bool):
        ...
        progress_bar_val = tqdm(total=len(val_loader),
                                desc=f"Validation RRDBNet Epoch {epoch+1}")

        with torch.no_grad():
            for batch_idx, batch_data in enumerate(val_loader):
                loss = self._perform_one_batch_step(
                    batch_data, predict_residual=predict_residual, is_training=False)
                total_val_loss += loss.item()
                num_val_batches += 1
                progress_bar_val.update(1)
                progress_bar_val.set_postfix(
                    val_loss_batch=f"{loss.item():.4f}")
        ...
        print(
            f"Epoch {epoch+1} Average Validation Loss (RRDBNet): {avg_val_loss:.4f}")
        if self.writer:
            self.writer.add_scalar(
                'Validation/Loss_epoch_avg', avg_val_loss, epoch + 1)
    ...
    def _log_rrdb_sample_images(self,
        ...
        if val_loader:
            try:
                ...
                print(
                    "Warning: val_loader is empty. Falling back to train_loader for image logging.")
                ...
                print(
                    f"Warning: Error with val_loader ({e}). Falling back to train_loader for image logging.")
                data_loader_for_samples = train_loader
                source_name = "Training"
        else:
            data_loader_for_samples = train_loader
        ...
        try:
            ...
            print(
                f"Warning: Could not get a sample batch from {source_name} loader for image logging.")
            self.model.train()
            return
        ...
        if self.writer:
            self.writer.add_image(
                f'Samples_from_{source_name}/01_Input_LR', grid_lr, epoch + 1)
            self.writer.add_image(
                f'Samples_from_{source_name}/02_{predicted_name}', grid_predicted, epoch + 1)
            self.writer.add_image(
                f'Samples_from_{source_name}/03_{target_name}', grid_target, epoch + 1)
            print(
                f"Logged sample RRDB images from {source_name} set to TensorBoard for epoch {epoch + 1}.")
    ...
    def train(self,
        ...
        if resume_checkpoint_path:
            self.start_epoch, self.global_step_optimizer, initial_best_loss_from_resume = self.load_checkpoint_for_resume(
                resume_checkpoint_path)
            self.best_loss = initial_best_loss_from_resume
            self.batch_step_counter = self.start_epoch * len(train_loader)

        print(
            f"Starting BasicRRDBNet training from epoch {self.start_epoch + 1}/{epochs} on device: {self.device}")
        print(f"Accumulation steps: {accumulation_steps}")
        if val_loader:
            print(
                f"Validation will be performed every {val_every_n_epochs} epoch(s).")
            print(
                f"Initial best tracked loss (from resume or inf): {self.best_loss:.6f}")
        else:
            print("No validation loader provided. 'best_loss' will track training loss.")
            print(
                f"Initial best training loss (from resume or inf): {self.best_loss:.6f}")
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            progress_bar = tqdm(total=len(train_loader),
                                desc=f"Train RRDBNet Epoch {epoch+1}/{epochs}")
            epoch_total_train_loss = 0.0
            num_batches_in_epoch = 0

            for batch_idx, batch_data in enumerate(train_loader):
                loss = self._perform_one_batch_step(
                    batch_data, predict_residual=predict_residual, is_training=True)
                ...
                if self.writer:
                    self.writer.add_scalar(
                        'Train/Loss_batch_step', loss_value, self.batch_step_counter)
                    if updated_optimizer_this_step and self.optimizer.param_groups:
                        self.writer.add_scalar(
                            'Train/LearningRate_optimizer_step', self.optimizer.param_groups[0]['lr'], self.global_step_optimizer)

                self.batch_step_counter += 1
                progress_bar.update(1)
                progress_bar.set_postfix(
                    loss=f"{loss_value:.4f}", opt_steps=self.global_step_optimizer, lr=f"{self.optimizer.param_groups[0]['lr']:.2e}")

            progress_bar.close()
            mean_train_loss_epoch = epoch_total_train_loss / \
                num_batches_in_epoch if num_batches_in_epoch > 0 else float(
                    'nan')
            print(
                f"Epoch {epoch+1} Average Training Loss (RRDBNet): {mean_train_loss_epoch:.4f}")
            if self.writer:
                self.writer.add_scalar(
                    'Train/Loss_epoch_avg', mean_train_loss_epoch, epoch + 1)
                if self.optimizer.param_groups:
                    self.writer.add_scalar(
                        'Train/LearningRate_epoch_end', self.optimizer.param_groups[0]['lr'], epoch + 1)
            ...
            if val_loader and (epoch + 1) % val_every_n_epochs == 0:
                avg_val_loss_epoch = self._run_validation_epoch(
                    val_loader, epoch, predict_residual)
            ...
            if current_loss_for_best_comparison < self.best_loss:
                ...
                self.save_checkpoint(epoch, self.best_loss, is_best_model=True,
                                     is_validation_loss=is_current_loss_validation)
                ...
                print(
                    f"Epoch {epoch+1}: {log_msg} to {self.best_checkpoint_path}")
            ...
            if (epoch + 1) % save_every_n_epochs == 0 or (epoch + 1) == epochs:
                if not (current_loss_for_best_comparison == self.best_loss and self.best_checkpoint_path == os.path.join(self.checkpoint_dir, f'rrdb_model_epoch_{epoch+1}.pth')):
                    self.save_checkpoint(
                        epoch, mean_train_loss_epoch, is_best_model=False, is_validation_loss=False)
            ...
            if self.scheduler:
                if isinstance(self.scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                    ...
                    print(
                        f"ReduceLROnPlateau scheduler stepped with metric {metric_for_plateau:.4f}. New LR: {self.optimizer.param_groups[0]['lr']:.2e}")
                elif not isinstance(self.scheduler, torch.optim.lr_scheduler.StepLR):
                    self.scheduler.step()
                    print(
                        f"Epoch-based scheduler ({type(self.scheduler).__name__}) stepped. New LR: {self.optimizer.param_groups[0]['lr']:.2e}")

            if self.writer:
                self._log_rrdb_sample_images(
                    train_loader, val_loader, epoch, predict_residual)

        if current_accumulation_idx > 0:
            print(
                f"Performing final optimizer step for {current_accumulation_idx} accumulated gradients...")
            self.optimizer.step()
            if self.scheduler and isinstance(self.scheduler, torch.optim.lr_scheduler.StepLR):
                self.scheduler.step()
            self.optimizer.zero_grad()
            self.global_step_optimizer += 1
            print(
                f"Final gradients applied. Total optimizer steps: {self.global_step_optimizer}")

        if self.writer:
            self.writer.close()
        print(
            f"BasicRRDBNet training finished. Final best tracked loss: {self.best_loss:.4f}")
        if val_loader:
    ...
    def save_checkpoint(self, epoch, loss_value, is_best_model=False, is_validation_loss=False):
        ...
        if is_best_model:
            ...
            save_path = os.path.join(
                self.checkpoint_dir, f'rrdb_model_epoch_{epoch+1}.pth')
    ...
    def load_checkpoint_for_resume(self, checkpoint_path: str):
        ...
        try:
            ...
            if loaded_trainer_configs:
                print(
                    f"Checkpoint was saved with trainer configs: {loaded_trainer_configs}")
                if self.model_config != loaded_trainer_configs.get('model_config'):
                    print(
                        "WARNING: Model configuration in checkpoint differs from current trainer's model configuration.")
            ...
            if 'model_state_dict' in checkpoint:
                ...
                if incompatible_keys.missing_keys:
                    print(
                        f"Resume Warning: Missing keys in model state_dict: {incompatible_keys.missing_keys}")
                if incompatible_keys.unexpected_keys:
                    print(
                        f"Resume Info: Unexpected keys in model state_dict from checkpoint: {incompatible_keys.unexpected_keys}")
            ...
            if 'scheduler_state_dict' in checkpoint and self.scheduler:
                try:
                    ...
                    print(
                        f"Warning: Could not load scheduler state: {e_sched_load}. Scheduler may start fresh.")

            start_epoch_res = checkpoint.get('epoch', -1) + 1
            global_step_optimizer_res = checkpoint.get(
                'global_step_optimizer', 0)

            loaded_loss_metric_from_checkpoint = checkpoint.get(
                'current_best_tracked_loss', checkpoint.get('loss', float('inf')))

            print(
                f"Resuming training from epoch {start_epoch_res}, Optimizer steps: {global_step_optimizer_res}")
            print(
                f"Loaded 'current_best_tracked_loss' (or 'loss') from checkpoint: {loaded_loss_metric_from_checkpoint:.4f}")
            if 'is_validation_loss' in checkpoint:
                print(
                    f"  The 'loss' value in the loaded checkpoint was a {'validation' if checkpoint['is_validation_loss'] else 'training'} loss.")
    ...
    def load_model_for_evaluation(model_path: str,
        ...
        if not os.path.isfile(model_path):
            print(
                f"Evaluation model checkpoint not found at {model_path}. Returning uninitialized model in eval mode.")
            return model.eval()
        ...
        model_config = checkpoint.get(
            'trainer_configs', {}).get('model_config', {})
        ...
        if nf is None or nb is None or sr_scale_val is None:
            raise ValueError(
                "model_config must contain 'num_feat', 'num_block', and 'sr_scale'.")
        ...
        if state_dict_to_load:
            ...
            if incompatible_keys.missing_keys:
                print(
                    f"Eval Load Warning: Missing keys in model: {incompatible_keys.missing_keys}")
            if incompatible_keys.unexpected_keys:
                print(
                    f"Eval Load Info: Unexpected keys in checkpoint: {incompatible_keys.unexpected_keys}")
            print("Model weights loaded successfully for evaluation.")
        else:
            print(
                f"Failed to load model weights for evaluation from {model_path}. Checkpoint format might be incorrect.")
