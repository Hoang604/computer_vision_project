...
class SongUNet(torch.nn.Module):
    def __init__(self,
                 ...
                 augment_dim=0,
                 # Auxiliary label dimensionality, 0 = no auxiliary labels.
                 aux_dim=0,
                 ...
                 resample_filter=[1, 1],
                 # If True, augment the latent variable.
                 aug=False,
                 # Dimension of the auxiliary latent variable.
                 aug_dim=32,
                 **kwargs
                 ):
        ...
        self.map_aux0 = Linear(
            in_features=aux_dim, out_features=noise_channels, **init_zero) if aux_dim else None
        self.map_aux1 = Linear(in_features=noise_channels,
                               out_features=noise_channels, **init_zero) if aux_dim else None
        self.map_aux2 = Linear(in_features=noise_channels,
                               out_features=noise_channels, **init_zero) if aux_dim else None
        self.map_aux3 = Linear(in_features=noise_channels,
                               out_features=noise_channels, **init_zero) if aux_dim else None
        ...
        if self.aug:
            nc_total = (np.array(channel_mult) *
                        (num_blocks + 1)).sum() * model_channels
            nc_total = nc_total - model_channels * \
                channel_mult[0] + model_channels
            self.nc_aug = model_channels
            self.conv_aug = Conv2d(in_channels=nc_total,
                                   out_channels=self.nc_aug, kernel=3, **init)
            self.map_aug0 = Linear(
                in_features=self.nc_aug, out_features=self.nc_aug, **init)
            self.map_aug1 = Linear(
                in_features=self.nc_aug, out_features=aug_dim * 2, **init)
    ...
    def forward(self, x, noise_labels=None, class_labels=None, augment_labels=None, aux_labels=None, return_norm=False):
        ...
        if self.map_aux0 is not None:
            assert aux_labels.shape[
                1] == self.aux_dim, f"Expected aux_labels to have {self.aux_dim} channels, got {aux_labels.shape[1]}"
        ...
        if self.aug:
            ...
            compact_feature = [F.interpolate(
                skip, size=skips[-1].shape[2:], mode='bilinear', align_corners=False) for skip in skips]
...
class EDMPrecond(torch.nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.img_resolution = model.config['img_resolution']
        self.img_channels = model.config['in_channels']
        self.label_dim = model.config['label_dim']
        self.augment_dim = model.config['augment_dim']
        self.use_fp16 = use_fp16
        self.sigma_min = sigma_min
        self.sigma_max = sigma_max
    ...
    def forward(self, x, sigma, class_labels=None, augment_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros(
            [1, self.label_dim], device=x.device) if class_labels is None else class_labels.to(torch.float32).reshape(-1, self.label_dim)
        ...
        F_x = self.model((c_in * x).to(dtype), c_noise.flatten(),
                         class_labels=class_labels, augment_labels=augment_labels, **model_kwargs)
        assert F_x.dtype == dtype
        D_x = c_skip * x + c_out * F_x.to(torch.float32)
        return D_x
...
# ----------------------------------------------------------------------------
# Velocity parameterization proposed in the paper "Improving the training of Rectified Flows"
...
class EDMPrecondVel(torch.nn.Module):
    ...
    def forward(self, x_, sigma_, class_labels=None, augment_labels=None, force_fp32=False, **model_kwargs):
        ...
        x = x.to(torch.float32)
        augment_labels = torch.zeros(
            [x.shape[0], self.augment_dim], device=x.device)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros(
            [1, self.label_dim], device=x.device) if class_labels is None else class_labels.to(torch.float32).reshape(-1, self.label_dim)
        dtype = torch.float16 if (
